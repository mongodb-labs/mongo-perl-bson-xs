...
sub encode_one {
    ...
    my $bson = eval { BSON::XS::_encode_bson( $document, $merged_opts ) };
    if ( $@
        or ( $merged_opts->{max_length} && length($bson) > $merged_opts->{max_length} ) )
    {
...
sub decode_one {
    ...
    my $document = eval { BSON::XS::_decode_bson( $string, $merged_opts ) };
    if ($@) {
        if ( $merged_opts->{error_callback} ) {
